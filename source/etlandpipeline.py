...
print("Reading reseller from Athena")
df_r = awswrangler.athena.read_sql_query("SELECT * FROM reseller", database="implementationdb")
print("Reading billing from Athena")
df = awswrangler.athena.read_sql_query("SELECT * FROM billing", database="implementationdb")
account_id = boto3.client('sts').get_caller_identity()['Account']
bucket = 'stepfunctions-' + str(account_id)
print("S3 bucket: " + str(bucket))
...
def write_dataframe_to_csv_on_s3(dataframe, bucket, filename):
    ...
    # Create buffer
    csv_buffer = StringIO()
    print("Write dataframe to buffer")
...
df = df[['bill', 'date', 'id_reseller', 'mean-last-30', 'mean-last-7',
       'std-last-30', 'days_without_purchase', 'weekday',
       'zone', 'cluster']]
